...
async def search_nearby_landmarks(lat: float, lon: float, keyword: str, radius_meters: int = 3000) -> List[Dict[str, Any]]:
    """
    ...
    # within the given radius of the centroid (lat, lon)
    query = f"""
    [out:json][timeout:5];
    (
    ...
    try:
        ...
        print(f"Overpass API timeout while searching for '{keyword}'")
